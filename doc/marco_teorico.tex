...
\mynote{Los siguientes modelos se caracterizan por ser de aprendizaje supervisado. Los datos de entrenamiento son presentados en pares, con caracaterísticas y etiquetas de identificación.}
...
\begin{equation}
	\mbox{Minimizar } C||w|| + \left[\dfrac{1}{n} \sum_{i=1}^{n} max\left(0,1-y_{i}\left(w^{T}x_{i}-b\right)\right)\right]
	\label{eqn:svm_soft_margin}
\end{equation}

\mynote{El desarrollo para llegar hasta \ref{eqn:svm_soft_margin} es mucho más extenso de lo presentado aquí (en verdad es la releche de largo xd) y no es el propósito de este trabajo.}
